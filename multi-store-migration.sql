...
-- PART 3: Add store_id to ALL data tables (NOT NULL on data tables)
-- ============================================================
...
-- PART 4: Backfill existing data to Lakeside (default store)
-- ============================================================
...
-- Check if user is admin (role = 'admin', store_id IS NULL = sees all stores)
...
-- next_invoice_no: global invoice counter (not per-store)
CREATE OR REPLACE FUNCTION public.next_invoice_no()
RETURNS text AS $$
  SELECT 'BM-AIS-' || lpad(
    COALESCE(
      (SELECT lpad(((regexp_replace(MAX(invoice_no), '[^0-9]', '', 'g'))::int + 1)::text, '4', '0') FROM public.sales),
      '0001'
    ), 4, '0'
  );
...
-- PART 7: New per-store RLS policies
-- ============================================================
-- Pattern: admin (store_id NULL) sees ALL; salesman sees own store only
...
CREATE POLICY stock_insert ON public.stock FOR INSERT TO authenticated
  WITH CHECK (store_id = user_store_id() OR user_store_id() IS NULL);
...
-- PART 9: Handle new user trigger
-- ============================================================
...
-- ============================================================
-- MIGRATION COMPLETE
-- Admin account: admin@bmstore.com / admin123 (role: admin, store_id: NULL)
-- Salesman accounts: sales1@, sales2@, sales3@bmstore.com / sales123
--   (role: salesman, store_id: respective store)
-- ============================================================
